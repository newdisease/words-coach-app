...
const InputBox = ({
  ...
  return (
    <span className={`quiz-input tac${answeredColorClass}`}>
      <input
        className="tac"
        type={type}
...
const QuizInput = ({ amount, replyStatus, setAnswer }) => {
  ...
  const handleChange = useCallback(
    ({ target }) => {
      if (target.value.match(/^[а-яА-Яa-zA-Z\sіІїЇєЄ`']+$/)) {
  ...
  return (
    <div className="expression-wrap flex">
      <div className="expression">{renderItems()}</div>
      <div className={`icon ${renderClass}`}>{renderIcon}</div>
    </div>
  );
};
